...
# Creating a heat map
ggplot(data_df, aes(x=hour, y=month, fill = solarenergy)) +
  geom_tile() +
  ...
  labs(title = "London solar energy in 2013",
       x = "Hour of the Day",
       y = "Month",
       fill = "Solar Energy "  ~ (MJ/m^2)) +
  theme_minimal() +
  theme(text = element_text(size = 11),
        legend.key.size = unit(1, 'cm')) 

# Save the plot as a PDF 
ggsave("heatmap_solarenergy_2013.pdf", width = unit(7, "cm"), height = unit(5, "cm"))